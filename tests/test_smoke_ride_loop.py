...
def test_full_ride_loop_and_admin(tmp_path, monkeypatch):
    ...
    with TestClient(main.app) as rider_c, TestClient(main.app) as driver_c, TestClient(
        main.app
    ) as admin_c:
        assert (
            rider_c.post(
        ...
        r = driver_c.post(f"/api/rides/{rid}/bids", json={"fare_cents": 1234})
        assert r.status_code == 200, r.text
        bid = r.json()
        bid_id = int(bid["id"])
        assert bid["distance_to_pickup_m"] > 0

        pins = rider_c.get(f"/api/rides/{rid}/bidder-locations")
        assert pins.status_code == 200, pins.text
        assert len(pins.json()) == 1
        assert pins.json()[0]["driver_id"] == bid["driver_id"]

        r = rider_c.post(f"/api/rides/{rid}/bids/{bid_id}/accept", json={})
        assert r.status_code == 200, r.text
        assert r.json()["status"] == "assigned"
        assert r.json()["final_fare_cents"] == 1234

        r = driver_c.post(f"/api/rides/{rid}/start", json={})
        assert r.status_code == 200, r.text
        assert r.json()["status"] == "in_progress"

        r = driver_c.post(f"/api/rides/{rid}/complete", json={})
        assert r.status_code == 200, r.text
        done = r.json()
        ...
        plot = admin_c.get("/api/admin/plots/revenue.png")
